...
const ClaimOperatorModal = ({ isOpen, onClose, existingOperators = [] }) => {
  ...
  useEffect(() => {
    const fetchRegionalOperators = async () => {
      try {
        ...
        const operators = [...new Set(games.map(game => game.competition))].filter(Boolean);
        
        // Add common fallbacks if none found
        const finalOperators = operators.length > 0 
          ? [...operators, 'Other (specify below)']
          : ['APL Poker', 'Australian Poker Experience', 'UPT Poker', 'Other (specify below)'];
          
        // Filter out any existing operators
        const filteredOperators = finalOperators.filter(op => !existingOperators.includes(op));
  ...
  const handleSubmit = async (e) => {
    ...
    const operatorName = selectedOperator === 'Other (specify below)' ? customOperator : selectedOperator;
    ...
    try {
      ...
      toast.success('Operator claim submitted! We\'ll review and get back to you within 2-3 business days.');
      handleClose();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-slate-800 rounded-lg max-w-lg w-full p-6 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="text-center mb-6">
          ...
          <h3 className="text-xl font-bold text-white mb-2">Claim Game Operator</h3>
          <p className="text-gray-300 text-sm">
            Are you a poker game operator? Claim your operator status to get verified and unlock promotion features.
          </p>
        </div>

        <div className="bg-blue-900 bg-opacity-30 border border-blue-600 rounded-lg p-3 mb-6">
          <h4 className="text-blue-400 font-semibold text-sm mb-2">✨ Benefits of Verification:</h4>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-yellow-900 bg-opacity-30 border border-yellow-600 rounded-lg p-3">
            <p className="text-yellow-400 text-xs">
              <strong>Note:</strong> We'll review your claim within 2-3 business days. 
              We may reach out using the contact information provided to verify your operator status.
            </p>
          </div>
