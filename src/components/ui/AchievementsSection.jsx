    ...
    <div className='achievements-section'>
        ...
        <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4'>
          <div className='flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4'>
            ...
            <p className='text-gray-400 text-sm text-left sm:text-left'>
              {earnedAchievements.length} of {Object.keys(ACHIEVEMENTS).length}{' '}
              earned
            </p>
          </div>
          ...
          <button
            ...
            className='btn btn-ghost btn-sm flex items-center gap-2 w-auto justify-center hidden sm:flex'>
      ...
      {earnedAchievements.length > 0 && !isExpanded && (
        <div className='flex items-start gap-3 mb-4'>
          ...
          <div className='relative flex-1 min-w-0'>
            ...
            <div className='absolute top-0 right-0 h-full w-8 bg-gradient-to-l from-[#1e293b] to-transparent pointer-events-none sm:hidden'></div>
          </div>
        </div>
      )}

      {/* Mobile View All Button (below badges when collapsed) */}
      {!isExpanded && (
        <div className='sm:hidden mb-4'>
          <button
            onClick={() => setIsExpanded(!isExpanded)}
            className='btn btn-ghost btn-sm flex items-center gap-2 w-full justify-center'>
            <FiChevronDown />
            View All Achievements
          </button>
        </div>
      ...
      {isExpanded && (
        <div>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-4 mb-6'>
              ...
              {profitProgress && (
                <div className='bg-gray-800 rounded-lg p-4 border border-gray-700'>
                  <div className='flex items-center justify-between mb-2'>
                    <span className='text-sm text-gray-300'>
                      Next Profit Milestone
                    </span>
                    <span className='text-xs text-gray-400'>
                      ${profitProgress.progress.toFixed(0)}/$
                      {profitProgress.target}
                    </span>
                  </div>
                  <div className='flex items-center gap-3'>
                    <AchievementBadge
                    ...
                    <div className='flex-1'>
                      ...
                      <div className='w-full bg-gray-700 rounded-full h-2'>
                        <div
                          ...
                          style={{
                            width: `${profitProgress.percentage}%`,
                          }}></div>
                      </div>
                    </div>
                  </div>
          ...
          <div className='flex flex-wrap gap-2 sm:gap-2 mb-6'>
            {categories.map(category => {
              const categoryAchievements = Object.values(ACHIEVEMENTS).filter(
                a => category.value === 'all' || a.category === category.value
              );
              ...
              return (
                <button
                  ...
                  className={`btn btn-sm flex items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm ${
          ...
          {/* Mobile Hide Button (at bottom when expanded) */}
